...
function TutorialContent({ provider, t }: { provider: MessagingProvider; t: (key: string) => string | undefined }) {
    ...
    return (
      <div className="space-y-3 pt-2">
        <TutorialStep number={1}>
          {t('channels.tutorial.telegram.step1') || 'Open Telegram and search for @BotFather'}
        </TutorialStep>
        <TutorialStep number={2}>
          {t('channels.tutorial.telegram.step2') || 'Send /newbot to create a new bot'}
        </TutorialStep>
        <TutorialStep number={3}>
          {t('channels.tutorial.telegram.step3') || 'Follow the prompts to set the bot name'}
        </TutorialStep>
        <TutorialStep number={4}>
          {t('channels.tutorial.telegram.step4') || 'Copy the Token you received'}
        </TutorialStep>
        <TutorialStep number={5}>
          {t('channels.tutorial.telegram.step5') || 'Paste the Token into the input field below'}
        </TutorialStep>
      </div>
    );
  }
  ...
  if (provider === 'discord') {
    return (
      <div className="space-y-3 pt-2">
        <TutorialStep number={1}>
          {t('channels.tutorial.discord.step1') || 'Visit the Discord Developer Portal'}
        </TutorialStep>
        <TutorialStep number={2}>
          {t('channels.tutorial.discord.step2') || 'Create a new application'}
        </TutorialStep>
        <TutorialStep number={3}>
          {t('channels.tutorial.discord.step3') || 'Add a Bot to your application'}
        </TutorialStep>
        <TutorialStep number={4}>
          {t('channels.tutorial.discord.step4') || 'Copy the Bot Token'}
        </TutorialStep>
        <TutorialStep number={5}>
          {t('channels.tutorial.discord.step5') || 'Invite the Bot to your server'}
        </TutorialStep>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3 pt-2">
      <TutorialStep number={1}>
        {t('channels.tutorial.default.step1') || 'Visit the platform developer portal'}
      </TutorialStep>
      <TutorialStep number={2}>
        {t('channels.tutorial.default.step2') || 'Create a new application or bot'}
      </TutorialStep>
      <TutorialStep number={3}>
        {t('channels.tutorial.default.step3') || 'Configure the required permissions'}
      </TutorialStep>
      <TutorialStep number={4}>
        {t('channels.tutorial.default.step4') || 'Copy the access credentials'}
      </TutorialStep>
      <TutorialStep number={5}>
        {t('channels.tutorial.default.step5') || 'Enter the credentials in the connection form'}
      </TutorialStep>
    </div>
  );
}
          ...
          <div className="border rounded-lg overflow-hidden">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                {t('channels.tutorial.title') || 'Connection Tutorial'}
              </span>
              {isTutorialOpen ? (
...
export default function ChannelsPage() {
  ...
  const pageTexts = {
    ...
    description: t('channels.description') || 'Manage messaging platform integrations',
    addChannel: t('channels.addChannel') || 'Add Channel',
    refresh: t('common.refresh'),
    noChannels: t('channels.noChannels') || 'No channels configured',
    connect: t('channels.connect') || 'Connect',
    disconnect: t('channels.disconnect') || 'Disconnect',
  };
